...
<p align="center">
  <img alt="Version" src="https://img.shields.io/badge/version-1.1.0-F4B942?style=flat-square">
  <img alt="Platform" src="https://img.shields.io/badge/platform-Windows%20%7C%20macOS%20%7C%20Android-41D3BD?style=flat-square">
...
织梭是一个开源的跨设备临时投递工具，支持 Windows、macOS 和 Android。把文字或文件放进投递箱，其他设备就能看到、复制或下载。用户既可以连接自己的固定服务器，也可以让一台电脑在局域网中托管；桌面端在没有服务器时，还可以建立跨网络的设备组。

“Meshuttle”由 **Mesh**（网状互联）和 **Shuttle**（织布梭）组合而成。数据像梭子一样在设备之间穿梭；设备组完成首次配对后，没有永久的“母机”，最初创建设备离线不会阻止其余在线设备继续同步。

> 当前版本：`1.1.0`　作者：**集御**　协议：自有代码采用 **MIT**，桌面端附带的 Syncthing 独立组件采用 **MPL-2.0**。

## 平台支持

| 平台 | 安装包 | 远程服务器 | 局域网主机 | 设备互联 | 批量下载与删除 |
| --- | --- | --- | --- | --- | --- |
| Windows 10/11 x64 | `Meshuttle-Setup-1.1.0.exe` | 支持 | 可作为主机 | 支持 | 支持 |
| macOS 12+ Intel / Apple 芯片 | `Meshuttle-1.1.0-macOS-universal.dmg` | 支持 | 可作为主机 | 支持 | 支持 |
| Android 8.0+ | `Meshuttle-1.1.0-Android.apk` | 支持 | 作为客户端连接 | 1.1.0 暂不支持 | 支持 |

Android 1.1.0 是原生客户端，可连接公网 Meshuttle Server 或桌面端在局域网中启动的本机服务。它支持文字、多个文件、批量下载、批量删除和安全保存访问码，但不会在手机里捆绑 Syncthing。无固定服务器的跨网络设备互联目前由 Windows 与 macOS 桌面端提供。
...
| 本地密钥保护 | 桌面端使用 Electron `safeStorage`，Android 使用 Android Keystore 加密保存访问码 |
...
先打开项目的 [Releases](../../releases/latest) 页面，并用发布页中的 `SHA256SUMS.txt` 核对下载文件。

### Windows

1. 下载 `Meshuttle-Setup-1.1.0.exe`。
2. 运行安装程序，选择安装目录并完成安装。
3. 首次启动后，从三种连接方式中选择一种。

当前 EXE 尚未使用受信任的 Authenticode 证书，Windows 可能显示“未知发布者”。请先核对哈希和源码，再决定是否运行。

### macOS

1. 根据 Releases 页面下载 `Meshuttle-1.1.0-macOS-universal.dmg`，Intel 与 Apple 芯片使用同一个安装包。
2. 打开 DMG，把“织梭”拖进“应用程序”。
3. 首次启动后选择远程服务器、本机托管或设备互联。

当前开源构建尚未使用 Apple Developer ID 签名和公证。如果 Gatekeeper 阻止首次打开，请先核对 SHA-256 与源码；确认可信后，在 Finder 中按住 Control 点击应用并选择“打开”。正式大范围分发前应配置 Apple 签名与公证。

### Android

1. 下载 `Meshuttle-1.1.0-Android.apk`。若发布页只有标有 `debug` 的 APK，它是可安装的测试签名构建，不适合作为长期更新渠道。
2. 在系统设置中仅为当前安装来源临时允许“安装未知应用”，安装后可关闭该权限。
3. 输入 Meshuttle Server 地址和至少 24 位访问码，点击“验证并保存”。公网地址必须使用 HTTPS；局域网内可以连接桌面端显示的 HTTP 地址。
4. 用“选择多个文件”发送文件，用内容列表的复选框批量下载或删除。

Android 不提供桌面悬浮投递窗，也不能在 1.1.0 中创建或加入 Syncthing 设备组。
...
1. 点击“添加文字”。
2. 输入或粘贴文字。
3. 点击“发送文字”，或者按 `Ctrl + Enter`（macOS 为 `Command + Enter`）。
...
- 桌面端：Node.js 22 或更新版本
- Windows 安装包：Windows 10/11 x64
- macOS 通用安装包：macOS 构建机
- Android APK：JDK 17、Gradle 9.1、Android SDK 36
...
```powershell
npm --prefix client run dist:win
```

在 macOS 构建 Intel / Apple 芯片通用安装包：
...
Android 正式发布签名通过 `ANDROID_KEYSTORE_PATH`、`ANDROID_KEYSTORE_PASSWORD`、`ANDROID_KEY_ALIAS` 和 `ANDROID_KEY_PASSWORD` 环境变量提供，私钥文件不得提交到 Git。桌面端输出位于 `client/release/`；Android 输出位于 `android/app/build/outputs/apk/`。
...
```text
meshuttle/
├─ client/                 Electron Windows / macOS 桌面端
│  ├─ build/               应用图标
│  ├─ p2p/                 Syncthing 控制、邀请和分布式存储
│  ├─ renderer/            主窗口、悬浮窗和设置界面
│  ├─ test/                存储、邀请和 UI 契约测试
│  └─ vendor/syncthing/    上游许可文件；可执行程序由脚本下载
├─ android/                原生 Android 客户端与构建配置
├─ server/                 固定服务器和接口测试
...
- 并发写入不依赖容易冲突的共享索引文件。
- 到期项目目录可以自动清理。
- Android API 契约、Bearer 鉴权和 Keystore 存储声明完整。
- GitHub Actions 分别构建 Windows、macOS 通用包和 Android APK。
...
- Windows / macOS 桌面端附带的 Syncthing 独立程序：[Mozilla Public License 2.0](https://www.mozilla.org/MPL/2.0/)
- Syncthing 版本、源码地址和归档哈希：[THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md)
- Android APK 不包含 Syncthing，也不包含 MPL-2.0 代码。
